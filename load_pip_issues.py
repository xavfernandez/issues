...
def output_close_speed_by_week(items):
    created_by_week = Counter(creation_week(item) for item in items)
    closed_by_week = Counter(close_week(item) for item in items)
...
def plot(items, item_type):
    result = output_close_speed_by_week(items)
